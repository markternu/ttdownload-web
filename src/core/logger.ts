...
export const MARKERS: Record<string, string> = {
  ...
  // —— BT（transmission）全流程 ——
  BT_SELECT: 'BT 只挑视频的结果（勾选几个 / 排除几个非视频）',
  BT_PICK: 'BT 多视频"挑同类"（命中哪条规则、下几个）',
  BT_PREPARE: 'BT 已备好（种子暂停在 transmission 里，等磁盘空间准入，含要下多少）',
  ...
  SETTINGS_MIGRATE: '旧配置自动修正（把历史遗留的错值改回正确值，会写清改了哪一项）',
