...
种子（尤其这类资源）里常混着宣传图、广告视频、网址文件。规则按**优先级**：

1. **广告关键词屏蔽**（最优先）：文件路径（含所在目录）命中关键词就排除。
   内置表：`广告 宣传 推广 加群 微信 二维码 扫码 电报 推特 官网 网址 必看 说明 试看 预告
   sample trailer preview promo advert screenshot screen cover poster thumb cover telegram`
   可在「设置 → BT 内容甄别」里改（**留空 = 用内置表**；填了就只用你填的）。
2. **图片策略**：默认 `auto` —— 种子里**有视频**就把图片当宣传图排除；整包都是图片
   （照片合集）时才保留。也可以强制「一律要」/「一律不要」。
3. **视频体积下限**：默认 **关闭**（`0`）。有些正片就是几十 MB，一刀切会误伤，
   确认需要再开。

> 每次挑片的决定（保留了哪些、排除了哪些、**为什么**）都会写进该任务的运行日志
> （标记 `BT_SELECT`）。觉得误伤了，照着日志把关键词删掉即可。

### 多个视频时挑哪些？（独树一帜，下最大；相差无几，一起下）

只下视频之后，如果种子里有**多个**视频，再按体积"挑同类、排异类"：

| 情形 | 例子 | 下什么 |
| --- | --- | --- |
| 只有一个视频 | `[800M]` | 直接下，没得判断 |
| 一个特别大 | `[2G, 200M, 150M]` | **只下 2G**（其他跟它差太远，是异类） |
| 几个大的 + 几个很小的 | `[2G, 1G, 160M, 100M]` | 下 **2G + 1G**（同类），小的排除 |
| "大的"其实不大、小的成堆 | `[190M, 30M, 25M, 20M, 15M]` | 下**那四个小的**（那个 190M 通常是片头/预告） |
| 体积都差不多 | `[500M, 480M, 460M]` | **全都下**（相差无几） |

三个数字都能在「设置 → BT 下载规则」里调：**相差多少倍算异类**（默认 5）、
**"伪大"的上限**（默认 200MB）、**"一堆小的"至少几个**（默认 3）。
每次筛选都会写进任务日志（`[MARK:BT_PICK]` + 逐条"不下（异类）: 文件名 —— 原因"）。

### 一个种子会产出几个成品？

按「单独发布阈值」（默认 **500MB**）拆：

- 单个视频 **≥ 阈值** → **各自一个成品**（单文件直接移动，不打包不压缩）。
  而且**一下完就提前交付**（不用等整个种子下完）：该文件会立刻归档→加密→出现在待下载列表，
  手机上可以先取走它。为了防止文件被移走后 transmission 重新校验又去重下，交付时会先把
  这个文件标成 unwanted。
- 其余小文件 → **等全部下完合成一个 zip**，再作为一个成品

所以一个含多个大视频的种子会产出多个待下载文件，安卓端各取各的；一堆几十 MB
的小视频则会打成一个包。
