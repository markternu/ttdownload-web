          ...
          <div className="space-y-4">
            <div>
              ...
              <p className="mt-1.5 text-[11px] text-slate-400">
                ...
                · <b>操作系统实际可用</b> {system ? formatBytes(system.disk.freeBytes, '未知') : '—'} ＝ 可用于下载{' '}
                {system ? formatBytes(system.disk.usableBytes, '未知') : '—'} ＋ 预留{' '}
                {system ? formatBytes(system.disk.reserveBytes, '0 B') : '—'}
              </p>
              <p className="mt-1 text-[11px] leading-4 text-slate-400">
                预留那份是留给「归档 → 加密 → 发布」等文件操作周转的（加密完成后才删源文件，
                所以这块空间一直留着）。<b>新下载能用的 = 操作系统实际可用 − 预留</b>。
                ⚠️ 不要把预留调小：空间被下载吃满时，加密 / 归档会因为没有周转空间直接失败。
